...
def create_comparison_hovertemplate(comparison_type):
    ...
    Args:
        comparison_type: Type of comparison ('hours', 'rate', or 'revenue')
...
def create_column_config(df):
    ...
    for col in df.columns:
        ...
        elif "Billability" in col:
            column_config[col] = st.column_config.NumberColumn(
                format="%.1f%%"  # Keep percentage with 1 decimal
            )
            
        # Currency formatting (rates and revenue)
        elif "rate" in col.lower() or col == "Revenue" or col == "Rate variance" or col == "Revenue variance" or col == "Planned revenue":
            # For currency, use simple format with symbol
            if position == 'before':
...
def get_comparison_type(comparison_pair):
    ...
    Returns:
        str: Comparison type ('hours', 'rate', or 'revenue')
    """
...
def apply_chart_style(fig, chart_type="default"):
    ...
    if chart_type != "default" and hasattr(fig, 'layout') and hasattr(fig.layout, 'title'):
        ...
        if current_title and ("Revenue" in current_title or "rate" in current_title.lower()):
...
def get_visualization_options(is_comparison_view=False, is_forecast_hours_view=False):
    ...
    if is_comparison_view:
        ...
        # For regular metrics, show all visualization options with Monthly first
        return ["Monthly: Bar chart", "Project: Treemap", "Project: Bar chart"]
