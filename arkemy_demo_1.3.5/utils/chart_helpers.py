...
def create_summary_metrics_table(monthly_data, has_planned_data=False):
    ...
    if "Total profit" in monthly_data.columns:
        ...
        if "Revenue" in summary_metrics and summary_metrics["Revenue"] > 0:
            summary_metrics["Profit margin %"] = (summary_metrics["Total profit"] / summary_metrics["Revenue"]) * 100
        else:
...
def create_yearly_metrics_table(monthly_data, filtered_df, has_planned_data=False):
    ...
    # Add revenue, cost, and profit aggregations
    if 'Revenue' in monthly_data.columns:
    ...
    if 'Total profit' in monthly_data.columns:
        ...
        if 'Revenue' in yearly_data.columns:
            ...
            mask = yearly_data['Revenue'] > 0
            yearly_data.loc[mask, 'Profit margin %'] = (yearly_data.loc[mask, 'Total profit'] / yearly_data.loc[mask, 'Revenue'] * 100)
