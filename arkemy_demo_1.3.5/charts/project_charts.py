...
def render_project_tab(filtered_df, aggregate_by_project, render_chart, get_category_colors, planned_df=None, filter_settings=None):
    ...
    # Identify planned-only metrics
    is_planned_only_metric = selected_metric in ["Planned hours", "Planned rate", "Planned revenue"]

    # Check if we have data for the selected view
    if filtered_df.empty and not is_comparison_view and not is_planned_only_metric:
    ...
    # For planned-only metrics with no actual data, check if we have planned data
    if filtered_df.empty and is_planned_only_metric and not has_planned_data:
        st.info("📊 No planned hours available for selected date range.")
        st.markdown("Try adjusting your date filter or check if planned data exists.")
        return
    ...
    if filtered_df.empty:
        ...
        project_agg = pd.DataFrame(columns=[
            "Project number", "Project", "Project type", "Hours worked", 
            "Billable hours", "Number of people", "Non-billable hours", 
            "Billability %", "Revenue", "Billable rate", "Effective rate"
        ])
    else:
